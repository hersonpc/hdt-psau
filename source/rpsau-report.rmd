...
for(selAno in unique(pesquisas$p1$ano)) { 
  ...
  for(selMes in unique(tmp$mes)) { 
    ...
    cat('\n')
    cat(paste0('## ', months_full[selMes], '/', selAno, '\n'))

    q1a <- table(pesquisas$p1 %>% 
                 filter(ano == selAno & mes == selMesInt) %>% 
                 select(a_1:h_7) %>% 
                 rowMeans(na.rm=T) %>% 
                 ceiling) %>% 
            melt
    q1b <- table(pesquisas$p2 %>% 
                 filter(ano == selAno & mes == selMesInt) %>% 
    ...
    t <- data.frame(
          ...
          Resultados = c(as.numeric(q1['total']), 
                         nrow(pesquisas$p1) + nrow(pesquisas$p2),
                         0
                        )
        )
    names(t) <- c(paste0('Indicadores - ', months_full[selMes], '/', selAno), "Resultados")
    ...
    consolidacoes(selAno, months_full[selMes])
  }
}
```
